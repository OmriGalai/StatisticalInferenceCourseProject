...
## Normality
Figure 1 contains a histogram and Q-Q plot of the sample distribution (see appendix).
The histogram is centered at the mean, is mound shaped and is visibly similar to the normal density we have overlaid.
On the Q-Q plot,  perfect normality would be a linear relationship between the normal quantiles (on the x axis) and the observed quantiles of the sample means (on the y axis). We observe an approximately linear relationship with more deviations toward the upper right corner implying a minimal skew to the right (based on [this blog post](https://towardsdatascience.com/q-q-plots-explained-5aa8495426c0).)
...
To sum up, we have seen that the distribution of 1000 samples we generated approximates the expected statistics very accurately and is very close to normal-shaped, as implied by the Central Line Theorem. Numeric evaluation exacerbates the visual impression on normality.
